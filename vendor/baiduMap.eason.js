        ...
        return {
            ...
            link: function($scope, element, attrs) {
                ...
                map.addEventListener("click", function(e){
                    ...
                    var mPoint = e.point;
                    circle = new BMap.Circle(mPoint,30,{fillColor:"blue", strokeWeight: 1 ,fillOpacity: 0.3, strokeOpacity: 0.3});
                ...
                $scope.$watch('options.city', function(newValue, oldValue) {
                    opts = $scope.options;
                    map.centerAndZoom(opts.city, opts.zoom);
                    mark();
                }, true);

                $scope.$watch('options.address', function(newValue, oldValue) {
                    opts = $scope.options;
                    var local = new BMap.LocalSearch(map, {
                        renderOptions:{map: map}
                    });
                    local.search(opts.address);
                    mark();
                }, true);
